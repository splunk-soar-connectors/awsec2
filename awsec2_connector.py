...
class AwsEc2Connector(BaseConnector):
    ...
    def _make_boto_call(self, action_result, method, **kwargs):
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"The boto3 call to ec2 failed. {error_message}"), None)
    ...
    def _create_client(self, service, action_result, param=None, region=None):
        ...
        try:
            if self._access_key and self._secret_key:
                ...
                self._service = client(service, region_name=region, config=boto_config)
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
    ...
    def _create_resource(self, action_result, resource_type, identifier=None, param=None):
        if resource_type not in EC2_RESOURCE_TYPES:
            return action_result.set_status(phantom.APP_ERROR, f"Incorrect resource type: {resource_type}")
        ...
        try:
            if self._access_key and self._secret_key:
                ...
                ec2 = resource("ec2", region_name=self._region, config=boto_config)
            ...
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Could not create boto3 instance: {error_message}")
    ...
    def _paginator(self, method_name, limit, action_result, key="Reservations", **kwargs):
        ...
        if (
            ...
            and EC2_PAGINATION_SUPPORTED_ACTIONS[self.get_action_identifier()] not in kwargs
        ):
    ...
    def _parse_filter_string(self, filters, action_result):
        try:
            ...
            error_message = self._get_error_message_from_exception(e)
            return action_result.set_status(phantom.APP_ERROR, f"Error occurred while parsing filter: {error_message}"), None

    def _parse_tag_specifications(self, tag_specs, action_result):
        try:
            return phantom.APP_SUCCESS, json.loads(tag_specs)
        except Exception as e:
            error_message = f"Error occurred while parsing tag specifications. {self._get_error_message_from_exception(e)}"
    ...
    def _handle_detach_instance(self, param):
        ...
        args = {"AutoScalingGroupName": autoscaling_group_name, "ShouldDecrementDesiredCapacity": should_decrement_desired_capacity}
        if instance_ids:
            args["InstanceIds"] = self._parse_comma_separated_ids(instance_ids)
    ...
    def _handle_attach_instance(self, param):
        ...
        args = {"AutoScalingGroupName": autoscaling_group_name}
        if instance_ids:
            args["InstanceIds"] = self._parse_comma_separated_ids(instance_ids)
    ...
    def _handle_deregister_instance(self, param):
        ...
        args = {"LoadBalancerName": load_balancer_name, "Instances": instance_id_dict}
    ...
    def _handle_create_security_group(self, param):
        ...
        args = {"GroupName": param["group_name"], "Description": param["group_description"]}
        if vpc_id:
            args["VpcId"] = vpc_id
        if dry_run:
            args["DryRun"] = dry_run
    ...
    def _handle_add_tag(self, param):
        ...
        args = {"Tags": [{"Key": tag_key, "Value": tag_value}]}
        if dry_run:
            args["DryRun"] = dry_run
    ...
    def _handle_get_tag(self, param):
        ...
        if not response.get("Tags"):
            resp_message = f"No tags found with the tag key: {tag_key} in the instance with ID: {instance_id}"
    ...
    def _handle_remove_tag(self, param):
        ...
        if not tag_key and tag_value:
            return action_result.set_status(
                phantom.APP_ERROR, "Providing tag value without a tag key performs nothing and hence, it is not allowed"
            )

        if tag_key and tag_value is None:
            tags_dict = {"Key": tag_key}
        elif tag_key:
            tags_dict = {"Key": tag_key, "Value": tag_value}
